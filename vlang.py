    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in " \t":
                ...
                tokens.append(Token(TT_VAR, pos_start=self.pos))
                self.advance()
...
class ParseResult:
    ...
    def try_register(self, res):
        if res.error:
            self.reverse_to_count = self.advance_count
            return None
...
class Parser:
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                if res.error:
                    return res.failure(InvalidSyntaxError(
                        self.current_tok.pos_start, self.current_tok.pos_end,
                        "Expected ')', '@', 'if', 'for', 'while', 'def', int, float, identifier, '+', '-', '(', '[', 'not'"
                    ))
    ...
    def if_cases(self, case_keyword):
        res = ParseResult()
        cases = []
        else_case = None
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            if self.current_tok.matches(TT_KEYWORD, "elif") or self.current_tok.matches(TT_KEYWORD, "else"):
                all_cases = res.register(self.if_b_or_c_expr())
                if res.error: return res
    ...
    def while_expr(self):
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            return res.success(WhileNode(condition, expr, True))
    ...
    def statement(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'return', 'continue', 'break', '@', 'if', 'for', 'while', 'def', int, float, identifier, '+', '-', '(', '[', 'not'"            ))
        return res.success(expr)
...
class Value:
    ...
    def __CompareEE__(self, other):
        return None, self.illegal_operation(other)
    def __CompareNE__(self, other):
        return None, self.illegal_operation(other)
    def __CompareLT__(self, other):
        return None, self.illegal_operation(other)
    def __CompareGT__(self, other):
        return None, self.illegal_operation(other)
    def __CompareLTE__(self, other):
        return None, self.illegal_operation(other)
    def __CompareGTE__(self, other):
        return None, self.illegal_operation(other)
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_int(self, exec_ctx):
        try:
            ...
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                f"Can't turn {exec_ctx.symbol_table.get('value')} into an integer",
                exec_ctx
            ))
    ...
    def execute_str(self, exec_ctx):
        try:
            ...
            print(f"Can't turn '{exec_ctx.symbol_table.get('value')}' into a string.")
    execute_str.arg_names = ["value"]

    def execute_isinstance(self, exec_ctx):
        object_ = exec_ctx.symbol_table.get("object")
        compare = exec_ctx.symbol_table.get("compare")
        if isinstance(compare, Number):
            if isinstance(object_, Number):
                return RTResult().success(Number.true)
            else:
                return RTResult().success(Number.false)
        elif isinstance(compare, String):
            if isinstance(object_, String):
                return RTResult().success(Number.true)
            else:
                return RTResult().success(Number.false)
            ...
            if isinstance(object_, List):
                ...
                return RTResult().success(Number.false)
        elif isinstance(compare, BaseFunction):
            if isinstance(object_, BaseFunction):
                return RTResult().success(Number.true)
            else:
                return RTResult().success(Number.false)
    execute_isinstance.arg_names = ["object", "compare"]
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_PLUS:
            ...
            result, error = left.__div__(right)
        elif node.op_tok.type == TT_POW:
            result, error = left.__pow__(right)

        elif node.op_tok.type == TT_EE:
            result, error = left.__CompareEE__(right)
        elif node.op_tok.type == TT_NE:
            result, error = left.__CompareNE__(right)
        elif node.op_tok.type == TT_LT:
            result, error = left.__CompareLT__(right)
        elif node.op_tok.type == TT_GT:
            result, error = left.__CompareGT__(right)
        elif node.op_tok.type == TT_LTE:
            result, error = left.__CompareLTE__(right)
        elif node.op_tok.type == TT_GTE:
            result, error = left.__CompareGTE__(right)
    ...
    def visit_UnaryOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_MINUS:
            number, error = number.__mul__(Number(-1))
    ...
    def visit_WhileNode(self, node, context):
        ...
        while True:
            ...
            if not condition.__true__(): break
...
global_symbol_table.set("List", List([]))
global_symbol_table.set("Number", Number(0))
global_symbol_table.set("String", String(""))
global_symbol_table.set("Function", BaseFunction(None))
